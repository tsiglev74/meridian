...
from google3.ads.lift.mmm.modeling.research.contr_priors.data import input_data as data
from google3.ads.lift.mmm.modeling.research.contr_priors.model import media
from google3.ads.lift.mmm.modeling.research.contr_priors.model import posterior_sampler
from google3.ads.lift.mmm.modeling.research.contr_priors.model import prior_distribution
from google3.ads.lift.mmm.modeling.research.contr_priors.model import prior_sampler
...
class Meridian:
  ...
  def _validate_data_dependent_model_spec(self):
    ...
    if self.model_spec.roi_calibration_period is not None:
      if (
          self.media_tensors.media_spend is not None
          and self.media_tensors.media_spend.ndim != 3
      ):
        raise ValueError(
            "`roi_calibration_period` must be `None` unless `media_spend` has"
            " geo and time dimensions."
        )
      if self.model_spec.roi_calibration_period.shape != (
          self.n_media_times,
          self.n_media_channels,
      ):
        raise ValueError(
    ...
    if self.model_spec.rf_roi_calibration_period is not None:
      if (
          self.rf_tensors.rf_spend is not None
          and self.rf_tensors.rf_spend.ndim != 3
      ):
        raise ValueError(
            "`rf_roi_calibration_period` must be `None` unless `rf_spend` has"
            " geo and time dimensions."
        )
      if self.model_spec.rf_roi_calibration_period.shape != (
          self.n_media_times,
          self.n_rf_channels,
      ):
        raise ValueError(
  ...
  def _warn_setting_ignored_priors(self):
    ...
    for ignored_priors_dict, prior_type, prior_type_name in (
        (
            constants.IGNORED_PRIORS_PAID_MEDIA,
  ...
  def linear_predictor_counterfactual_difference_media(
    ...
    """Calculates the difference between transformed and counterfactual media."""
  ...
  def linear_predictor_counterfactual_difference_rf(
    ...
    """Calculates the difference between transformed and counterfactual RF."""
  ...
  def calculate_beta_x(
    ...
    if random_effects_dist == constants.MEDIA_EFFECTS_NORMAL:
